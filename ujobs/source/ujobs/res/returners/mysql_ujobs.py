...
.. code-block:: sql
    ...
    CREATE TABLE `salt_returns` (
      ...
      `full_result` mediumtext NOT NULL,
...
__virtualname__ = 'mysql_ujobs'

_ignore_funcs = ["saltutil.find_job","test.ping","grains.items","saltutil.running"]
...
def returner(ret):
    ...
    '''
    try:
        if ret['fun'] in _ignore_funcs:
            return
        with _get_serv(ret, commit=True) as cur:
            sql = '''INSERT INTO `salt_returns`
                    (`fun`, `jid`, `result`, `minion_id`, `success`, `full_result`,`ctime` )
...
def save_load(jid, load):
    ...
    with _get_serv(commit=True) as cur:
        ...
        try:
            cur.execute(sql, (jid, json.dumps(load),datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
...
def get_jid(jid):
    ...
    with _get_serv(ret=None, commit=True) as cur:
        ...
        sql = '''SELECT minion_id, full_result FROM `salt_returns`
                WHERE `jid` = %s'''
...
def get_fun(fun):
    ...
    with _get_serv(ret=None, commit=True) as cur:
        ...
        sql = '''SELECT s.minion_id,s.jid, s.full_result
                FROM `salt_returns` s
