...
from releaseinfo import (DATABASE_ENGINE, DATABASE_NAME, DATABASE_USER,
                         ...
                         REL_CACHE_BACKEND,REL_CACHE_TIME,PLATFORM_HOST)
...
SECRET_KEY = 'yqb7a7pys5^1k0&yn3ck_nnc2ew=ka0&mban$xp+-3yuab@6-&'
...
LOGGING = {
    ...
    'handlers': {
        ...
        'default': {
            ...
            'filename': os.path.join(REL_SITE_ROOT + '/logs/', 'kaisa.log'),
            'when': 'MIDNIGHT',
            'formatter':'standard'
        },
        'console':{
        ...
        'request_handler': {
            'level':'DEBUG',
            'class':'logging.handlers.TimedRotatingFileHandler',
            'filename': os.path.join(REL_SITE_ROOT + '/logs/', 'request.log'),
            'when': 'MIDNIGHT',
            'formatter':'standard'
        },
        'scprits_handler': {
            'level':'DEBUG',
            'class':'logging.handlers.TimedRotatingFileHandler',
            'filename': os.path.join(REL_SITE_ROOT + '/logs/', 'script.log'),
            'when': 'MIDNIGHT',
            'formatter':'standard'
        },
        'usual_handler': {
            'level':'DEBUG',
            'class':'logging.handlers.TimedRotatingFileHandler',
            'filename': os.path.join(REL_SITE_ROOT + '/logs/', 'usual.log'),
            'when': 'MIDNIGHT',
            'formatter':'standard'
        },
        'agent_handler': {
            'level':'DEBUG',
            'class':'logging.handlers.TimedRotatingFileHandler',
            'filename': os.path.join(REL_SITE_ROOT + '/logs/', 'agent.log'),
            'when': 'MIDNIGHT',
            'formatter':'standard'
        },
        'api_handler': {
    ...
    'loggers': {
        ...
        'logger_usual': { # logger for usual operation.
             'handlers': ['usual_handler'],
             'level': 'DEBUG',
             'propagate': False
         },
        'logger_agent': { # logger for agent operation.
              'handlers': ['agent_handler'],
              'level': 'DEBUG',
              'propagate': False
       },
        'logger_files': { # logger for files operation.
              'handlers': ['default','console'],
              'level': 'DEBUG',
              'propagate': False
        },
        'logger_utils': { # logger for utils operation.
              'handlers': ['default','console'],
              'level': 'DEBUG',
              'propagate': False
        },
...
SCHEDULER_CONFIG = {
    'apscheduler.jobstores.default': {
        'type': 'sqlalchemy',
        'url': 'mysql://{0}:{1}@{2}/{3}'.format(DATABASE_USER, DATABASE_PASSWORD, DATABASE_HOST, DATABASE_NAME)
    },
...
SPECIAL_USERS = ['shenjh','wx','luozh','zhangyanl']