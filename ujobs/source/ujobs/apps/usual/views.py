...
from apps.template.models import TemplateStepScript, TemplateStep
import simplejson
    ...
    try:
        ...
        for obj_id in obj_ids:
            ...
            if auth_type == PERM_PTYPE_TEMPLATE:
                #获取所有该模板的脚本
                steps =  TemplateStep.objects.filter(template__id=obj_id,step_type=enum_template.STEP_TYPE_SCRIPT,is_delete=False)
                script_steps = TemplateStepScript.objects.filter(step__in=steps)
            ...
            for user_name in user_names:
                ...
                if auth_type == PERM_PTYPE_TEMPLATE:
                    for script in scripts:
                        p,is_create = Perm.objects.get_or_create(ptype=PERM_PTYPE_SCRIPT,object_id=script.id,is_delete=False,create_user=user,to_user=to_user)
                        logger.debug("create auth for script:{0} to user:{1},is_create:{2}".format(script.id,to_user.username,is_create))
            ...
            if discard_perms:
                ...
                if auth_type == PERM_PTYPE_TEMPLATE:
                    ...
                    for script_id in script_ids:
                        rel_script_steps = TemplateStepScript.objects.filter(version__script__id=script_id,step__is_delete=False)
                        rel_template_ids = [script_step.step.template.id for script_step in rel_script_steps]
                        for name in names:
                            # 存在其他关联模板授权的不进行回收该脚本.
                            if Perm.objects.filter(ptype=PERM_PTYPE_TEMPLATE,object_id__in=rel_template_ids,is_delete=False,to_user__username=name).count()>0:
                                logger.debug("other template using this template, no revoke,user:{0},script_id:{1}".format(name,script_id))
                            else:
                                logger.debug("revoke auth, script_id:{0}, user:{1}".format(script_id,name))
                                Perm.objects.filter(ptype=PERM_PTYPE_SCRIPT,object_id=script_id,is_delete=False,create_user=user,to_user__username=name).update(is_delete=True)
