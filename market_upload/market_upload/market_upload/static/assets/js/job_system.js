...
jQuery(document).ready(function($) {
    ...
    // 显示百分比
    function showProgress(fileId, dataId, fileSize, fileName, productId) {
        var obj = $('#' + fileId);

        var time = setInterval(function() {
            $.ajax({
                url: '/product/ajax_get_upload_info/',
                type: 'POST',
                dataType: 'json',
                data: {
                    info_id: dataId,
            ...
            .done(function(data) {
                if (data.result === 1) {
                    if (data.percent !== 100) {
                        obj.find('.percent').text(data.percent);
                    } else {
                        obj.find('.load-msg').empty().append('<input type="text" class="url" title=' + data.file_url + ' value=' + data.file_url + '><span class="pull-right"><a class="copy" href="javascript:void(0);" onclick="javascript:copyToClipboard(this);">复制链接</a></span>');
