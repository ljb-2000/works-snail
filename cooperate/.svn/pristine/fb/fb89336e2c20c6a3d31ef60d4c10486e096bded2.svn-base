...
@csrf_exempt
@commit_on_success
def ajax_delete_monthly(request):
    sdicts = {'result':0}
    monthly_id = request.POST.get("id",'')
...
def ajax_get_more_tasks(request):
    ...
    try:
        product = _report.get_product_by_params(name=product_name)
        taskCircles = _report.get_taskCircles_by_params(product=product, start_time__lt=saturday).order_by('-id')
...
def ajax_edit_task(request):
    ...
    try:
        ...
        task_circle = _report.get_or_create_taskCircle_by_params(product=product, start_time=saturday, end_time=friday)[0]
...
urlpatterns = patterns('',
    ...
    url(r'^report/ajax_delete_monthly/$', ajax_delete_monthly, name='ajax_delete_monthly'),
